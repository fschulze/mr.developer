...
class Extension:
    ...
    def get_threads(self):
        threads = int(
            self.buildout["buildout"].get(
                "mr.developer-threads", self.get_config().threads
            )
        )
        return threads
    ...
    def get_mrdev_verbose(self):
        return (
            self.buildout["buildout"].get("mr.developer-verbose", "").lower() == "true"
        )
        ...
        for name in section:
            ...
            for option in options:
                ...
                if key in source:
                    ...
                    logger.info(f"Overriding '{key}' for source '{name}'.")
                if key == "path":
    ...
    def get_auto_checkout(self):
        ...
        auto_checkout = set(self.buildout["buildout"].get("auto-checkout", "").split())
        if "*" in auto_checkout:
