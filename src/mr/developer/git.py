...
class GitWorkingCopy(common.BaseWorkingCopy):
    ...
    def git_checkout(self, **kwargs):
        ...
        if os.path.exists(path):
            self.output(
                (logger.info,
                 "Skipped cloning of existing package '%s'." %
                 name))
            return
    ...
    def git_switch_branch(self, stdout_in, stderr_in):
        ...
        if cmd.returncode != 0:
            raise GitError(
                "git checkout of branch '%s' failed.\n%s" %
                (branch, stderr))
        return (stdout_in + stdout,
                stderr_in + stderr)
    ...
    def update(self, **kwargs):
        ...
        if not self.matches():
            self.output(
                (logger.warning,
                 "Can't update package '%s' because its URL doesn't match." %
                 name))
