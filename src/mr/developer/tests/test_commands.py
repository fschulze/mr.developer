...
class TestCommand:
    ...
    def testSingleArgMatchingMultipleAuto(self, command):
        pytest.raises(SystemExit, command.get_packages, ["ba"], auto_checkout=True)
    ...
    def testArgsMatchingMultipleAuto(self, command):
        pytest.raises(
            SystemExit, command.get_packages, ["ba", "zap"], auto_checkout=True
        )
...
class TestDeactivateCommand:
    ...
    def develop(self, develop):
        develop.sources = dict(
            foo=MockSource(), bar=MockSource(), baz=MockSource(), ham=MockSource()
        )
        develop.auto_checkout = {"foo", "ham"}
    ...
    def testDeactivateDeactivatedPackage(self, cmd, develop):
        ...
        assert develop.config.develop == dict(bar=False, foo="auto", ham="auto")
    ...
    def testDeactivateAutoCheckoutPackage(self, cmd, develop):
        ...
        try:
            ...
            _logger.__exit__(None, None, None)
        assert develop.config.develop == dict(foo=False, ham="auto")
        assert logger.mock_calls == [
